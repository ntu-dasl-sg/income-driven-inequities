...
# Check if 'x' and 'y' coordinates are the same for every row
coordinates_match <- all.equal(income_pts[, c("x", "y")], AL50_pts[, c("x", "y")]) # check building_pts as well. This code cannot compare 3 data frames.
...
# Check if 'x' and 'y' coordinates are the same for every row
coordinates_match <- all.equal(WL50_pts[, c("x", "y")], income_pts[, c("x", "y")]) # check building_pts as well. This code cannot compare 3 data frames.
...
data_WL50 %>%  
  ...
  mutate(income_category = ifelse(income <= pov_line_intl, "Below poverty line", "Above poverty line")) %>% 
   ggplot(aes(x=welfare_loss, linetype = income_category))+ 
  # stat_ecdf(geom = "step", size = 0.5) +
  geom_smooth(stat = "ecdf", method = "loess", color = "black", se = FALSE, size = 0.5) +  # Apply loess smoothing
  labs(x = "Welfare losses ('000 PHP)", 
       y = "Cumulative frequency", 
...
loss_data_20 %>%
  ...
  labs(x = "Losses ('000 PHP)",
       ...
       linetype = "Income category",
       color = "Loss type",
       title = "Asset and welfare losses for households below poverty line") +
  # scale_linetype_manual(values = c("Below poverty line" = "dashed", "Above poverty line" = "solid")) +
  scale_color_manual(values = c("#cf5519", "#294475")) +
  theme_bw()
```
...
loss_data_20 %>%
  ...
  labs(x = "Losses ('000 PHP)",
       ...
       linetype = "Income category",
       color = "Loss type",
       title = "Asset and welfare losses for households abovepoverty line") +
  # scale_linetype_manual(values = c("Below poverty line" = "dashed", "Above poverty line" = "solid")) +
  scale_color_manual(values = c("#cf5519", "#294475")) +
  theme_bw()
```
...
```{r}
data_WL50 <- data_WL50 %>% rename(losses = welfare_loss)
...
loss_data_50 <- bind_rows(mutate(loss_data_20 %>% 
                   filter(loss_category == "Asset loss"),
                 loss_category = factor(loss_category), 
                 colour = "#cf5519"),
          mutate(loss_data_20 %>% 
...
loss_data_50 %>% 
  ...
  labs(x = "Losses ('000 PHP)",
       ...
       linetype = "Income category",
       color = "Loss type", 
       title = "Losses experienced by households below poverty line") +
  # scale_linetype_manual(values = c("Below poverty line" = "dashed", "Above poverty line" = "solid")) +
  scale_color_manual(values = c("#cf5519", "#294475")) +
  theme_bw()
```
...
loss_data_50 %>% 
  ...
  labs(x = "Losses ('000 PHP)",
       ...
       linetype = "Income category",
       color = "Loss type", 
       title = "Losses experienced by households above poverty line") +
  scale_linetype_manual(values = c("Below poverty line" = "dashed", "Above poverty line" = "solid")) +
  scale_color_manual(values = c("#cf5519", "#294475")) +
  theme_bw()
```
...
asset_loss_data %>%
  ...
  theme_bw()
```
### 7.2 COMPARE WELFARE LOSSES (BY YEAR)
