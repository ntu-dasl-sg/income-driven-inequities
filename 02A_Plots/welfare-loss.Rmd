...
t = 0:15 # time period after disaster where we want to track recovery
...
# yearly_savings <- seq(10, 350, length.out = length(tot_incomes))
# tot_savings <- yearly_savings * 2.9 # assume same savings rates throughout
...
for (i in 1:length(predisaster_res$labour_income)) {
  # should just run this in a script
  labour_income <- predisaster_res$labour_income[i]
  total_savings <- predisaster_res$tot_savings[i]
  replacement_cost <- predisaster_res$replacement_cost[i]
  income0 <- predisaster_res$income0[i]
  consumption0 <- predisaster_res$consumption0[i]
  utility0 <- predisaster_res$utility0[i]
  ...
  # Loop over damage fractions and find optimal rate that minimizes utility losses
  for (j in seq_along(damage_fractions)){
    damage_fraction <- damage_fractions[j]
  ...
  result_row <- cbind(income = rep(labour_income, length(results$damage_fraction)), results)
...
ggplot(aes(x = income, y = depth, fill = utility_loss)) + 
  ...
  labs(x = "Income ('000 PHP)", y = "Flood Depth (cm)", fill = "Welfare Losses ('000 PHP)") +
  theme_bw()+
...
ggplot(aes(x = income, y = depth, fill = asset_loss)) + 
  ...
  scale_fill_gradientn(colors = viridis_pal()(100),
                       breaks = pretty_breaks(), 
                       trans = "reverse") +  # Use the viridis color palette, pretty breaks for rounded numbers
  labs(x = "Income ('000 PHP)", y = "Flood Depth (cm)", fill = "Asset Losses ('000 PHP)") +
  theme_bw()+
  guides(fill = guide_colorbar(reverse = TRUE))
```
...
results %>% 
  filter(income <= 600) %>% 
ggplot(aes(x = income, y = depth, fill = consumption_loss)) + 
  geom_raster() + 
  scale_fill_gradientn(colors = viridis_pal()(100),
                       breaks = pretty_breaks(), 
                       trans = "reverse") +  # Use the viridis color palette, pretty breaks for rounded numbers
  labs(x = "Income ('000 PHP)", y = "Flood Depth (m)", fill = "Consumption Losses ('000 PHP)") +
  theme_bw()+
  guides(fill = guide_colorbar(reverse = TRUE))
```
...
results %>% 
 filter(income <= 600) %>% 
ggplot(aes(x = income, y = depth, fill = ratio)) + 
  geom_raster() + 
  scale_fill_gradientn(colors = viridis_pal()(100),
                       breaks = pretty_breaks(), 
                       trans = "reverse") +  # Use the viridis color palette, pretty breaks for rounded numbers
...
ggplot(aes(x = income, y = depth, fill = asset_loss/utility_loss)) + 
  ...
  labs(x = "Income ('000 PHP)", y = "Flood Depth (cm)", fill = "Asset to welfare loss ratio") +
  theme_bw() 
...
# results %>%
#   filter(income <= 650) %>%
#   ggplot(aes(x = income, y = depth, z = asset_loss, color = ..level..)) +
#   geom_contour(aes(fill = stat(level))) +
#   # geom_contour_filled(aes(fill = as.numeric(stat(level)))) +  # Convert level to numeric
#   scale_color_gradientn(colors = viridis_pal()(100),
#                        breaks = pretty_breaks(),
#                        trans = "reverse") +
#   labs(x = "Income ('000 PHP)", y = "Flood Depth (m)", color = "Welfare Losses ('000 PHP)") +
#   theme_light() +
#   guides(color = guide_colorbar(reverse = TRUE))

results %>% 
  filter(income <= 600) %>% 
  ggplot(aes(x = income, y = depth, z = asset_loss)) + 
  geom_contour_filled() + 
...
results %>% 
  filter(income <=650) %>% 
  ggplot(aes(x = income, y = depth, z =utility_loss)) + 
  geom_contour_filled() + 
  scale_fill_viridis_d(direction = -1) +
  labs(x = "Income ('000 PHP)", y = "Flood Depth (cm)", fill = "Welfare Losses ('000 PHP)") +
  theme_bw()
...
```{r contour al-wfl ratio}
results %>% 
  filter(income <= 600) %>% 
  ggplot(aes(x = income, y = depth, z = asset_loss/utility_loss)) + 
  geom_contour_filled() + 
  scale_fill_viridis_d(direction = -1) +
  labs(x = "Income ('000 PHP)", y = "Flood Depth (cm)", fill = "Asset to Welfare Loss Ratio") +
  theme_bw()
