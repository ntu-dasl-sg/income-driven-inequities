...
ggplot(AAL, aes(x = factor(year), y = sum_loss/1000000, fill = loss_type)) +
  ...
  labs(x = "Year", 
       ...
       fill = "Loss type") +
  scale_fill_manual(values = c("Asset loss" = "#cf5529", "Welfare loss" = "#294475")) +
  theme_bw()
```
...
AAL_increase %>% 
  ...
  labs(x = element_blank(),
       y = "Increase in avereage annual asset and welfare losses (billion PHP)", 
       fill = "Loss type")
```
...
## 3. GET WELFARE LOSSES

### 3.1. PREDISASTER CALC
...
### 3.2 OPTIMISE RECOVERY RATES & COMPUTE WELFARE LOSS
...
### 3.3. COMPUTE ASSET AND CONSUMPTION LOSSES
...
ggplot(results, aes(x = income, y = depth, fill = utility_loss)) + 
  geom_tile() + 
  ...
  labs(x = "Income ('000 PHP)", y = "Flood Depth (m)", fill = "Welfare Losses") +
  theme_bw()+
